...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    //    그래서 서버는 클라이언트가 요청을 할 때마다 누구인지 알수없다.
    // => 이를 해결하기 위해 클라이언트가 접속하면 웹 서버는 그 클라이언트를 위한
    //    고유 번호를 발급(쿠키 이용) 한다.
    //    이 고유번호를 '세션아아디'라 부른다.
    //    웹 브라우저는 세션아이디를 보관해 두었다가 그 서버에 요청할 때마다 세션 아이디를 보낸다.
    //    왜? 세션 아이디는 쿠키이다.
    // => 세션 아이디 쿠키는 유효기간을 설정하지 않았기 때문에
    //    웹 브라우저를 종료하면 세션아이디 쿠키는 삭제된다.
    //    세션 아이디 쿠키의 사용범위는 웹 애플리케이션 이다.
    //    예) /java-web
    //    따라서 같은 웹 애플리케이션의 서블릿을 실행할 때는 무조건 세션 아이디를 보낸다.
    // 세션 아이디는 언제 발급하는가?
    // => 새 세션을 생성할 때 아이디를 발급한다.
    // 언제 새 세션을 생성하는가?
    // => 세션이 없는 상태에서 request.getSession()을 호출할 때 생성한다.
    // => HTTP 요청 프로토콜    
    /*   
    HTTP/1.1 200 
    Set-Cookie: JSESSIONID=6203E4463B59C32E74D00073B0C766B6; Path=/java-web; HttpOnly
    Content-Type: text/plain;charset=UTF-8
    Content-Length: 43
    Date: Wed, 03 Apr 2019 02:38:18 GMT
    */
    ...
    //    - 있으면 그 세션을 리턴한다.
    //    - 있는데 유효기간이 지난 세션 객체라면, 새로 세션을 만들어 리턴한다.
    //    - 없다면, 새로 세션을 만들어 리턴한다.
