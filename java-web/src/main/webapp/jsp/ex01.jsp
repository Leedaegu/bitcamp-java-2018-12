...
2) 서블릿 컨테이너가 실행
    ...
    2.4 JSP 파일이 변경된 상태라면,
      2.4.1 다시 "2.3"을 반복한다.  예) http://localhost:8080/java-web/jsp/ex01.jsp
      
3) 서블릿 컨테이너 ==> 웹브라우저
    - 서블릿 실행 결과를 응답
    
JSP 파일을 가지고 생성한 서블릿 소스(.java) 와 클래스 파일(.class)의 위치
- org.eclipse.wst.server.core/temp(x)/work/...(.java)
...
JSP 실행
- JSP 파일이 있는 위치를 지정한다.  예) http://localhost:8080/java-web/jsp/ex01.jsp

  예) http://localhost:8080/java-web/jsp/ex01.jsp
  
JSP 파일을 변경한 후 실행을 요청하기
- 그냥 JSP 파일이 있는 위치를 지정하면 된다.
- 위에서 설명한대로 JSP 구동원리에 따라 동작한다.
