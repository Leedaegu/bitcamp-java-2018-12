...
public class Controller03_2 {
  ...
  // => 요청 헤더 중에서 특정 이름을 갖는 헤더가 있을 떄 호출될 메서드를 지정할 수 있다.
  // => 웹 페이지에서 링크를 클릭하거나 입력폼에 값을 넣고 등록 버튼을 누르는
  ...
  @GetMapping(headers = "age") // age라는 파라미터 값이 있으면 handler2()가 호출된다.
  @ResponseBody 
  public String handler2() {
    return "handler2";
  }

  @GetMapping(headers ={"age", "name"}) // 파라미터가 여러개일때는 배열로 지정하면 된다.
  @ResponseBody 
