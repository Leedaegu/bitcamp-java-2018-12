...
public class Controller03_1 {
  ...
  @GetMapping(params = "name") // name이라는 파라미터 값이 있으면 handler1()이 호출된다.
  @ResponseBody 
  ...
  @GetMapping(params = "age") // age라는 파라미터 값이 있으면 handler2()가 호출된다.
  @ResponseBody 
  ...
  @GetMapping(params ={"age", "name"}) // 파라미터가 여러개일때는 배열로 지정하면 된다.
  @ResponseBody 
