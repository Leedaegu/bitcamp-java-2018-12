...
public class Controller05_3 {
  ...
  public Object handler1(
    ...
    return String.format("d ,%s ,%s ,%d", no ,title ,wirter ,viewCount);
  }
  ...
  // 3) JSON 형식의 요청파라미터 값을 통째로 문자열로 받기
  //     테스트:
  ...
  public Object handler3(@RequestBody String content) throws Exception {
    System.out.println(content); // 디코딩 하기전
    System.out.println(URLDecoder.decode(content, "UTF-8")); // 디코딩 한 후
    return "Ok!";
  }

  // 4) JSON 형식의 요청파라미터 값을 맵 객체로 받기
  ...
  // 5) JSON 형식의 요청파라미터 값을 도메인 객체로 받기
  // => HttpMessageConverter 구현체(MappingJackson2HttpMessageConverter)가
  //     클라이언트가 보낸 데이터를 도메인 객체(Board, Lesson, Member)에 담아준다.
  //  => Json객체의 프로퍼티명과 도메인 객체의 프로퍼티명이 일치해야 한다.
  //
  //     테스트:
  //         http://.../html/app2/c05_3.html
  @RequestMapping(value="h5", produces="text/plain;charset=UTF-8")
  public String handler5(@RequestBody Board content) throws Exception {
    System.out.println(content);
    ...
    //   GsonHttpMessageConverter는 이 애노테이션을 인식하지 못한다.
