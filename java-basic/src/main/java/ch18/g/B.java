// 기존 코드에 영향을 미치지 않고 새 규칙을 추가하는 방법 
package ch18.g;
...
public interface B {
  ...
  //    문재는 새 인터페이스를 만들게 되면 기존 코드도 변경해야 하는 번거로움이 있었다.
  // => 이런 문제를 해결하기위해 등장한 문법이 디폴드 메서드이다.
  //    즉 인터페이스도 구현 메서드를 가질 수 있다.

  // 다음과 같이 새 추상메서드를 추가하면 기존의 My1, My2클래스에서 오류가 발생한다!
  // void m2();
