...
public class Test13 {
  ...
  public static void main(String[] args) {     
    ...
    // 그 외 다른 패키지의 멤버를 사용할 떄는 반드시 패키지 이름을 함께 지정해야 한다.
    ...
    // y : 날짜 및 시각 데이터에서 년도를 추출하여 뒤의 2자리로 표현한다.
     ...
     // 한국은 긴이름 짧은 이름이 없기때문에 같게출력. ex) 1월, 1월
     System.out.printf("%1$tB, %1$tb\n", today);
    
     // m : 날짜 및 시각 데이터에서 월을 추춸하여 2자리 숫자로 표현한다. ex)12.01
     ...
     // H : 날짜 및 시각 데이터에서 시각을 추출하여 24시 로 표현한다 
     // I : 날짜 및 시각 데이터에서 시각을 추출하여 12시 로 표현한다.
     ...
     // p : 오전 오후 출력하기\
     //      소문자 t를 사용하면 ap pm 으로 출력하고,
     //      대문자 T를 사용하면 AM PM 으로 출력한다.
     //      한글은 대소문자가 없기 때문에 의미없다.
     System.out.printf("%1$tp\n", today);
     
     //년-월-일-시:분:초를 출력한라! 예)2019-01-04 12:04:30
     System.out.printf("%1$tY-%1$tm-%1$td %1$tI:%1$tM:%1$tS \n", today);
