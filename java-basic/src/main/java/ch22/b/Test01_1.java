...
public class Test01_1 {
  ...
  public static void main(String[] args) {
    ...
    // => 메모장으로 편집이 불가능한 파일.
    // => 보통 전용 프로그래을 이용해서 편집하는 파일.
    // => 예) .hwp, .gif, .jpg, .mp3, .mp4, .ppt, .xls, .doc,
    //      .class, .swf, .avi 등
    
    // 텍스트 에디터?
    // => 특정 character set(문자표) 규칙에 따라 작성한 파일
    // => 메모장으로 편집 가능.
    // => 예) .txt, .c, .java, .cpp, .xml, .html, .css, .js
    //      .py, .docx, .xlsx, .pptx 등
    ...
    out.write(102);
    out.write(127);
    out.write(0x11223344); // 00010001 00100010 00110011 01000100 // 44
    out.write(0x55667788); // 맨끝 1바이트만 출력한다.                    // 88
