...
public class Test06 {
  ...
  // 인터페이스도 static 메서드를 가질수 있다.
  // => 특정 인터페이스에 종속되지 않고 작업하는 경우에 static 메서드로 정의한다.
  static interface Player2 {
    ...
    // 인터페이스에 스태틱 메서드나 디폴트 메서드가 여러개 있더라도
    // 그 메서드들은 이미 구현된 메서드이기 때문에 추상 메서드와 상관 없다.
    // 따라서 이 인터페이스는 추상 메서드가 한개다.
    // 즉 람다로 구현할 수 있다.
    void play();
  }

  // 추상메서드가 두개 이상이면 람다 문법으로 구현할 수 없다.
