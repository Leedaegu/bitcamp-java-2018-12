  ...
  public static void main(String[] args) {
    ...
    try {      
      ...
      for (int i = 0; i < 1000; i++) {
        ...
        // 만약 Heap 역역의 메모리가 모두 사용된다면
        // 더이상 배열을 생성할수 없기 때문에 "시스템 예외" 가 발생할 것이다.
      }
    ...
    // 시스템 예외가 발생 했을때 위와 같이 적절하게 조치를 취하고 
    // 다음과 같이 계속 실행할 수 있지만,
    ...
    //  이런 상황이 "시스템 예외"이다.
    //  따라서 "시스템 예외" 가 발생한다면 즉시 현재 애플리케이션의 작업내용을 저장하고,
    //  예외에 대한 기록을 로그 파일에 남긴 다음에 프로그램을 종료하는 것이 최선의 방법이다!
    
    list.add(new long[1000000]); // 메모리가 부족한 상태이기 때문에 애플리케이션에서 처리 불가능!
